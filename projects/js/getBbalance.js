...
var HowManyRepeatUnrecognize = 0;
	...
	//+++++++++++++++++++Setings
	var LIST_file="AllB";// where get adreses
	var total_file="totalB.csv"; // where put recived data
	var log_file = "logB.log"; // 
	//-------------------Setings
...
function GetBalanceFromBCI(){
	var adr=ara[k];
	var page = require("webpage").create();
	var get_data="https://blockchain.info/ru/q/addressbalance/"+adr;// <-------------- API 
	//console.log("Phantomjs.Cook get_data="+get_data);
...
fs.write(total_file,"Finde none zero balance at k="+k+" Adr="+adr+" balance="+balance+"\n", 'a');
					...
					fs.write("totalB.csv",i+";"+k+";"+adr+";"+bal+";"+total_balance+"\n", 'a');
					var balance=parseInt(bal);
					//console.log("Phantomjs. balance  after parse");
